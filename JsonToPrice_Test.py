...
def exceltojson():
    ...
    excel = base_dir + '\\reports\\price.xls'
    if not os.path.exists(excel):
...
def TestInterface():
    ...
    excel = base_dir + '\\reports\\经销商订货表.xls'
    if not os.path.exists(excel):
        CopyFile(srcexcelfile, excel)
    else:
        os.remove(excel)
    ...
    print(pictureliststring)
    closexls()
    #picturelist = json.loads(pictureliststring)

#     excel = base_dir + '\\reports\\转换前.xls'    #转换前010
#     openxls(excel)
#     printarea = {
#   "PrintArea": "A单生产表!$A$1:$M$13",
#   "Scale": 80,
#   "PaperSize": 9,
#   "HL": "",
#   "HC": "",
...
#   "FR": "&D &T  \n第 &P 页，共 &N 页"
# }
#     SetPrintArea(1, json.dumps(printarea, ensure_ascii=False))
    # print(picturelist)
